...
begin
  FreeAndnil(LogDataset);
  FreeAndnil(ComandDataset);
  FreeAndnil(UserIDList);
  inherited;
end;

procedure TLogFrame.ComboNivelDrawItem(Control: TWinControl; Index: Integer;
  Rect: TRect; State: TOwnerDrawState);
var
...
procedure TLogFrame.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumn; State: TGridDrawState);
var
  TempImg: Graphics.TBitmap;
...
procedure TLogFrame.ComboUsuarioChange(Sender: TObject);
begin
...
begin
  ...
  if ComboUsuario.ItemIndex > 0 then
    Temp := Temp + ' and ' + FTabLog + '.idUser = ' + UserIDList[ComboUsuario.ItemIndex];
...
procedure TLogFrame.Data1Change(Sender: TObject);
begin
  btfiltro.Enabled := True;
end;

procedure TLogFrame.btfiltroClick(Sender: TObject);
begin
  AplicaFiltro;
end;

procedure TLogFrame.AplicaFiltro;
var
  FTabUser: string;
  FTabLog: string;
  Temp: string;
begin
  ...
  LogDataset := FUserControl.DataConnector.UCGetSQLDataset(Temp);
  DataSource1.DataSet := LogDataset;
  btexclui.Enabled := not LogDataset.IsEmpty;
end;
...
begin
  ...
  if Assigned(UserIDList) = False then
  ...
  with FUserControl do
    if ((FUserControl.CurrentUser.Privileged = True) or
      ...
      User)) then
      begin
        ComandDataset := DataConnector.UCGetSQLDataset
  ...
  while not ComandDataset.EOF do
    begin
      ComboUsuario.items.Append(ComandDataset.FieldByName('Nome').AsString);
      UserIDList.Append(ComandDataset.FieldByName('idUser').AsString);
      ComandDataset.Next;
    end;
  ...
  with FUserControl do
    begin
      ...
      {$ENDIF}
      LogDataset := DataConnector.UCGetSQLDataset(SQLStmt);
    end;
  DataSource1.DataSet := LogDataset;
  btexclui.Enabled := not LogDataset.IsEmpty;
