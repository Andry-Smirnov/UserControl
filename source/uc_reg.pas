...
procedure Register;
procedure ShowControlsEditor(Component: TUCControls);
{$IFNDEF FPC}
procedure ShowUserControlsEditor(Component: TUserControl);
{$ENDIF}
...
begin
  ...
  with TUCObjSel.Create(nil) do
    begin
      ...
      // TUCControls(Component).ListComponents(FForm.Name, FInitialObjs);
      lbGroup.Caption := FUCControl.GroupName;
      // TUCControls(Component).GroupName;
      ShowModal;
      Free;
    end;
...
{  try
  ...
  if FModulo = nil then
  begin
    ShowMessage('M�dulo ' + FUCControl.UserControl.Owner.Name + ' n�o encontrado!');
    Exit;
  end
  else
