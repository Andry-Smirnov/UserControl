{ *************************************************************
  ...
  Contributor   Fran�ois PIETTE (http://www.overbyte.be)
...
type
  ...
  TALCPUVendor = (cvUnknown, cvAMD, cvCentaur, cvCyrix, cvIntel, cvTransmeta,
  ...
  TALCPUInstructions = (isFPU, { 80x87 }
    isTSC, { RDTSC }
    isCX8, { CMPXCHG8B }
  ...
  { ----------------------- }
  TAlSmtpClientAuthType = (AlsmtpClientAuthNone, alsmtpClientAuthPlain,
    AlsmtpClientAuthLogin, AlsmtpClientAuthCramMD5, AlsmtpClientAuthCramSha1,
  ...
  published
    ...
    property DispositionNotificationTo: String Read fDispositionNotificationTo
      Write fDispositionNotificationTo;
  ...
  public
    ...
    function GetAuthTypeFromEhloResponse(EhloResponse: String)
      : TAlSmtpClientAuthTypeSet; virtual;
...
var
  WSAData: TWSAData;
  hostEnt: PHostEnt;
{$IFDEF DELPHI12_UP} addr: PAnsiChar; {$ELSE} addr: PChar; {$ENDIF}
begin
  WSAData.wVersion := 0;
  WSAStartup(MAKEWORD(2, 2), WSAData);
  try
...
// ***********************************************************************
const
  CAlRfc822DaysOfWeek: array [1 .. 7] of String = ('Sun', 'Mon', 'Tue', 'Wed',
    'Thu', 'Fri', 'Sat');
...
begin
  ...
  Result := Format('%s, %.2d %s %.4d %s %s',
    [CAlRfc822DaysOfWeek[DayOfWeek(aValue)], aDay, CALRfc822MonthNames[aMonth],
...
// ***********************************************************************
const
  cALMime_Base64_Encode_Table: array [0 .. 63] of byte = (065, 066, 067, 068,
    069, 070, 071, 072, // 00 - 07
...
procedure ALMimeBase64EncodeNoCRLF(const InputBuffer;
...
{$IFDEF DELPHI12_UP} PSrc, PRes: PAnsiChar; {$ELSE} PSrc, PRes: PChar; {$ENDIF}
  IgnoreCase: Boolean;
begin
...
{$IFDEF DELPHI12_UP}
        CharReplace[IgnoreCase](Result, PAnsiChar(OldPattern[1]),
          PAnsiChar(NewPattern[1]));
{$ELSE}
          ...
          begin
            ...
            begin { Create or Expand Dynamic Buffer }
              ...
              begin { Create Dynamic Buffer }
                ...
                ALStringReplaceMoveEx(StaticBuffer, Buffer^,
                  SizeOf(StaticBuffer));
              end
...
begin
  ...
  if (SourceStringLength = 0) or (aLength < 1) or
...
begin
  ...
  if Trim(fContentTransferEncoding) <> '' then
    Result := Result + 'Content-Transfer-Encoding: ' +
      Trim(fContentTransferEncoding) + #13#10;
  ...
  if Trim(fDispositionNotificationTo) <> '' then
    Result := Result + 'Disposition-Notification-To: ' +
      Trim(fDispositionNotificationTo) + #13#10;
  for I := 0 to FCustomHeaders.Count - 1 do
    if (Trim(FCustomHeaders.names[I]) <> '') and
      (Trim(FCustomHeaders.ValueFromIndex[I]) <> '') then
      Result := Result + FCustomHeaders.names[I] + ': ' +
        Trim(FCustomHeaders.ValueFromIndex[I]) + #13#10;
end;
...
begin
  ...
  begin
    case ErrCode of
      ...
      WSAENOTSOCK:
        S := 'Socket operation on nonsocket';
      WSAEDESTADDRREQ:
      ...
      WSAESHUTDOWN:
        S := 'Cannot send after socket shutdown';
      WSAETIMEDOUT:
      ...
      WSASYSNOTREADY:
        S := 'Network subsystem is unavailable';
      WSAVERNOTSUPPORTED:
        S := 'Winsock.dll version out of range';
      WSANOTINITIALISED:
        S := 'Successful WSAStartup not yet performed';
      WSAEDISCON:
        S := 'Graceful shutdown in progress';
      WSAHOST_NOT_FOUND:
        S := 'Host not found';
      WSATRY_AGAIN:
        S := 'Nonauthoritative host not found';
      WSANO_RECOVERY:
        S := 'This is a nonrecoverable error';
      WSANO_DATA:
      ...
      begin
        ...
        FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM or FORMAT_MESSAGE_FROM_HMODULE,
          pointer(GetModuleHandle('wsock32.dll')), ErrCode, 0, PChar(S),
          Length(S), nil);
    ...
    raise Exception.Create(Format('%s (Error code:%s)', [S, IntToStr(ErrCode)])
      ); { Do not localize }
  end;
end;
...
{$IFDEF DELPHI12_UP}
    CheckError(setsockopt(FSocketDescriptor, SOL_SOCKET, SO_RCVTIMEO,
      PAnsiChar(@Ftimeout), SizeOf(Integer)) = SOCKET_ERROR);
    CheckError(setsockopt(FSocketDescriptor, SOL_SOCKET, SO_SNDTIMEO,
      PAnsiChar(@Ftimeout), SizeOf(Integer)) = SOCKET_ERROR);
{$ELSE}
    CheckError(setsockopt(FSocketDescriptor, SOL_SOCKET, SO_RCVTIMEO,
      PChar(@Ftimeout), SizeOf(Integer)) = SOCKET_ERROR);
    CheckError(setsockopt(FSocketDescriptor, SOL_SOCKET, SO_SNDTIMEO,
      PChar(@Ftimeout), SizeOf(Integer)) = SOCKET_ERROR);
{$ENDIF}
...
{ EhloResponse is like:
  ...
  250-8BITMIME
  250 SIZE 0 }
function TAlSmtpClient.GetAuthTypeFromEhloResponse(EhloResponse: String)
...
{ ----------------------------------- }
  ...
  begin
    ...
    aAuthPlain := ALMimeBase64EncodeStringNoCRLF(AUserName + #0 + AUserName + #0
      + APassword);
...
begin
  ...
  case tmpAuthType of
    alsmtpClientAuthPlain:
      Result := InternalDoAuthPlain;
    AlsmtpClientAuthLogin:
      Result := InternalDoAuthLogin;
    AlsmtpClientAuthCramMD5:
      raise Exception.Create('CRAM-MD5 Authentication is not supported yet!');
    AlsmtpClientAuthCramSha1:
      raise Exception.Create('CRAM-SHA1 Authentication is not supported yet!');
  else
...
begin
  ...
  begin
    ...
    Result := Result + SendCmd('RCPT To:<' + aRcptNameValue + '>', [250, 251]
      ) + #13#10;
  end;
  if Result <> '' then
...
begin
  ...
  begin
    if (aMailData[I] = '.') and (aMailData[I - 1] = #10) and
      (aMailData[I - 2] = #13) then
      Insert('.', aMailData, I);
    Inc(I);
  end;
...
var
  ...
  ByteSent: Integer;
begin
  if (Length(aCmd) <= 1) or (aCmd[Length(aCmd)] <> #10) or
...
begin
  ...
  begin
    ...
    begin
      ...
      // The maximum total length of a reply line including the reply code and the <CRLF> is 512 Characters. (http://www.freesoft.org/CIE/RFC/821/24.htm)
      aBuffStrLength := SocketRead(aBuffStr[1], Length(aBuffStr));
      aResponse := aResponse + ALCopyStr(aBuffStr, 1, aBuffStrLength);
      if aResponse = '' then
        raise Exception.Create('Connection close gracefully!');

      if (aBuffStrLength > 1) and (aBuffStr[aBuffStrLength] = #10) and
